...
export class Processing extends Construct {
    ...
    constructor(scope: Construct, id: string, props: ProcessingProps) {
        ...
        const { prod, invoiceBucket, invoiceTable, processingJobTable, exportTable, insightTable } =
            props;
        ...
        const commonEnv: Record<string, string> = {
            ...
            EXPORT_TABLE: exportTable.tableName,
        ...
        const makeQueue = (name: string) => {
            ...
            dlq.metricApproximateNumberOfMessagesVisible({
                ...
                threshold: 1,
                evaluationPeriods: 1,
                alarmDescription: `Messages in ${name} DLQ`,
                treatMissingData: cloudwatch.TreatMissingData.NOT_BREACHING,
            });
            queue
                .metricApproximateAgeOfOldestMessage({ period: cdk.Duration.minutes(5) })
                .createAlarm(this, `${name}QueueAgeAlarm`, {
                    threshold: 600,
                    evaluationPeriods: 2,
                    alarmDescription: `Messages in ${name} queue are aging`,
                    treatMissingData: cloudwatch.TreatMissingData.NOT_BREACHING,
                });
        ...
        // S3 key format changed: invoices/original/{teamId}/{fileObjectId}
